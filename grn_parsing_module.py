...
class GRNResponse(BaseModel):
    ...
    warehouse_or_location: Optional[str] = None
...
PROMPT = """Extract Goods Receipt Note (GRN) details and return ONLY valid JSON with fields:
- grn_number, grn_date, po_number, supplier_name, delivery_challan_number, warehouse_or_location,
- line_items: array of {item_code, description, quantity_received, uom, batch_or_lot, remarks},
...
class GRNFieldGenerator:
    def __init__(self):
        if not LLM_CONFIG.get("api_key"):
            raise ValueError("GROQ_API_KEY not set in environment.")
        self.agent = ConversableAgent(
            name="grn_field_generator",
            system_message="You extract structured GRN (Goods Receipt Note) data. Return valid JSON only.",
